...
공개 URL에서 실행/변경 작업을 보호하려면 `ADMIN_TOKEN`을 설정합니다. 값이 비어 있으면 로컬 개발처럼 보호가 꺼지고, 값이 있으면 조회용 `GET` API는 검수할 수 있도록 열어두고 수동 실행, 관심 등록 변경, DB 이관 같은 `POST` API만 보호합니다.

```powershell
$env:ADMIN_TOKEN="long-random-admin-token"
```

Render에서는 Dashboard의 Environment에 `ADMIN_TOKEN`을 비밀값으로 입력합니다. 대시보드 조회는 토큰 없이 가능하며, 스케줄러 수동 실행이나 관심 등록 변경처럼 상태를 바꾸는 작업을 하려면 `관리자 토큰` 카드에 같은 값을 한 번 입력하면 됩니다. 브라우저는 이후 보호된 API 요청에 다음 헤더를 붙입니다.
...
대시보드 조회는 기본적으로 `candidate_data_snapshots`에 저장된 후보별 최신 수신 데이터를 먼저 사용합니다. 뉴스 발굴 저장본은 가격과 등락률이 확인된 후보만 보조로 쓰며, 페이지 새로고침은 후보 재발굴이 아니라 저장 결과 조회로 동작합니다. 새 후보 수집은 스케줄러, 발굴 봇, `POST /api/discovery/run`, 또는 `/api/dashboard?refresh=1`에서만 수행됩니다. 뉴스 발굴 저장본을 대시보드 1순위로 쓰려면 `SIGNAL_DASHBOARD_STORED_DISCOVERY_FIRST=1`로 켭니다.
...
기본 화면 목표는 국내 20개와 해외 20개지만, 봇의 실제 발굴·저장 대상은 `SIGNAL_DISCOVERY_SELECTION_LIMIT`까지 넓게 유지합니다. 관련 뉴스가 확인된 1차 후보를 우선 선별하고, 부족한 경우 후보 풀과 유니버스 관심도까지 계속 재검토합니다. 화면의 `진입` 탭은 전체 후보 중에서도 현재가, 준비도, 리스크, 과열도를 다시 통과한 후보만 보여 주므로 목록이 적거나 비어 있을 수 있습니다.

```powershell
$env:SIGNAL_AUTO_CANDIDATES_ENABLED="1"
$env:SIGNAL_AUTO_CANDIDATE_LIMIT="40"
$env:SIGNAL_DISCOVERY_SELECTION_LIMIT="120"
$env:SIGNAL_DISCOVERY_EXPANDED_SELECTION_LIMIT="240"
$env:SIGNAL_DOMESTIC_CANDIDATE_LIMIT="20"
$env:SIGNAL_OVERSEAS_CANDIDATE_LIMIT="20"
$env:SIGNAL_DISCOVERY_MAX_SYMBOLS="320"
$env:SIGNAL_DISCOVERY_EXPANDED_MAX_SYMBOLS="640"
$env:SIGNAL_DISCOVERY_POOL_ACTIVE_TARGET="160"
$env:SIGNAL_DISCOVERY_VISIBLE_DOMESTIC_TARGET="20"
...
실전 운영 기준에서는 DB 연결을 필수 전제로 봅니다. 파일 저장소는 개발·임시 fallback 용도이며, Render 재배포나 재시작 뒤 후보 풀, Toss 최신값, 성과 기록 보존을 장담할 수 없습니다. 설정 화면의 `스냅샷 저장소` 카드에서 `운영 저장소`가 `DB 기준`으로 표시되어야 서버 수집 데이터가 안정적인 기준 저장소에 남는 상태입니다.

설정 화면의 저장소 표시는 다음 기준으로 읽습니다. `분석 저장소`가 `임시 저장값만 있음`이면 화면 표시용 데이터는 있지만 재배포/재시작 후 유실될 수 있으므로 실전 판단 기준이 아닙니다. `실전 판단`이 `가능`으로 바뀌려면 후보 데이터와 최신 Toss 수집값이 모두 Postgres DB에서 읽혀야 합니다.
...
- 정규화된 뉴스 이벤트: `signal_news_events`
...
뉴스는 원천 응답과 별도로 `signal_news_events`에 정규화해 저장합니다. 서버는 네이버/GDELT에서 받은 기사 제목, 요약, 출처, URL, 발행/수집 시각, 관련 종목, 관련성 메타데이터를 후보 판단용 이벤트로 남기고, 화면은 최신 DB 스냅샷을 읽어 뉴스 시그널과 가격 반응을 연결합니다. 본문 전체를 복제하지 않고 판단에 필요한 메타데이터 중심으로 저장합니다.
...
$env:SIGNAL_CANDIDATE_POOL_MAX_ITEMS="1000"
$env:SIGNAL_CANDIDATE_POOL_RETAIN_LIMIT="80"
$env:SIGNAL_CANDIDATE_POOL_SCAN_LIMIT="400"
...
$env:SIGNAL_CANDIDATE_PREFETCH_LIMIT="50"
$env:SIGNAL_CANDIDATE_PREFETCH_MAX_LIMIT="60"
...
$env:SIGNAL_FINAL_DECISION_STABILITY_ENABLED="1"
