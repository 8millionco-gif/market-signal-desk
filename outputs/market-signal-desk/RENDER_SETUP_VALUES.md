...
메인 화면의 `후보 풀` 카드는 DB에 누적된 상시 감시 후보 중 현재 살아 있는 후보 수, 진입/검증/눌림 상태, 상위 후보, 사후 성과를 보여줍니다. `성과` 화면을 열면 스냅샷 기준가와 현재가를 비교한 결과가 후보 풀에 누적되고, 승률과 평균 변화가 다음 후보 압축·정렬에 제한적으로 반영됩니다. 리스크와 가격 반응 기준을 통과하지 못한 종목을 매수 후보로 강제로 올리지는 않습니다.
