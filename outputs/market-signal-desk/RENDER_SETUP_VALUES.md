...
`분석 저장소`가 `임시 저장값만 있음`으로 표시되면 화면에는 최근 후보가 보일 수 있지만, 서버 재시작 뒤 보존이 보장되지 않습니다. 실제 투자 판단에 사용할 기준은 `실전 판단=가능`, `후보 기준=DB 기준`, `최신값 기준=DB 기준` 세 항목이 동시에 충족된 상태입니다.
...
SIGNAL_RAW_EVENT_FILE_LIMIT=300
SIGNAL_RAW_EVENT_FILE_MAX_LIMIT=300
SIGNAL_NEWS_EVENT_STORAGE_ENABLED=1
SIGNAL_NEWS_EVENT_FILE_LIMIT=400
SIGNAL_NEWS_EVENT_FILE_MAX_LIMIT=400
SIGNAL_NEWS_EVENT_MAX_ITEMS=1200
SIGNAL_NEWS_EVENT_ITEMS_MAX_LIMIT=1200
```

앱은 DB 연결이 가능하면 `signal_kv`, `signal_snapshots`, `signal_raw_events`, `signal_news_events` 테이블을 자동 생성합니다. `SIGNAL_DB_AUTO_MIGRATE=1`이면 기존 파일 저장소에 남은 후보 풀, 최신 발굴 결과, 최근 스냅샷을 DB에 한 번 자동 이관합니다. 설정 화면의 `스냅샷 저장소` 카드에서 `DB 이관 실행`을 누르면 같은 이관을 수동으로 재확인할 수 있습니다.
...
signal_news_events      종목별 정규화 뉴스 이벤트와 관련성 메타데이터
```
...
원천 이벤트 저장은 이후 데이터 신뢰도, 후보 선정 사유 추적, 백테스트 검증에 쓰입니다. 운영에서는 `SIGNAL_RAW_EVENT_STORAGE_ENABLED=1`과 `SIGNAL_NEWS_EVENT_STORAGE_ENABLED=1`을 유지하고, payload가 과도하게 커지면 `SIGNAL_RAW_EVENT_PAYLOAD_LIMIT` 값을 낮춥니다.
...
GDELT_BACKOFF_SECONDS=900
NAVER_NEWS_CANDIDATES_MAX_LIMIT=2
```
...
SIGNAL_AUTO_CANDIDATE_LIMIT=40
SIGNAL_DISCOVERY_SELECTION_LIMIT=120
SIGNAL_DISCOVERY_EXPANDED_SELECTION_LIMIT=240
SIGNAL_DOMESTIC_CANDIDATE_LIMIT=20
SIGNAL_OVERSEAS_CANDIDATE_LIMIT=20
SIGNAL_DISCOVERY_MAX_SYMBOLS=320
SIGNAL_DISCOVERY_EXPANDED_MAX_SYMBOLS=640
SIGNAL_DISCOVERY_POOL_ACTIVE_TARGET=160
SIGNAL_DISCOVERY_VISIBLE_DOMESTIC_TARGET=20
SIGNAL_DISCOVERY_SCAN_ROTATION_ENABLED=1
SIGNAL_LIVE_PRICE_SYMBOL_LIMIT=30
SIGNAL_LIVE_PRICE_SYMBOL_MAX_LIMIT=30
SIGNAL_DISCOVERY_NEWS_DISPLAY=3
SIGNAL_DISCOVERY_CACHE_SECONDS=600
SIGNAL_DASHBOARD_STORED_DISCOVERY_FIRST=0
...
SIGNAL_CANDIDATE_POOL_MAX_ITEMS=1000
SIGNAL_CANDIDATE_POOL_RETAIN_LIMIT=80
SIGNAL_CANDIDATE_POOL_SCAN_LIMIT=400
SIGNAL_CANDIDATE_POOL_RETAIN_MIN_SCORE=58
SIGNAL_CANDIDATE_POOL_TOP_LIMIT=5
SIGNAL_CANDIDATE_PREFETCH_ENABLED=1
SIGNAL_CANDIDATE_PREFETCH_LIMIT=50
SIGNAL_CANDIDATE_PREFETCH_MAX_LIMIT=60
SIGNAL_CANDIDATE_PREFETCH_INTERVAL_SECONDS=180
SIGNAL_CANDIDATE_PREFETCH_MIN_INTERVAL_SECONDS=180
SIGNAL_CANDIDATE_DATA_STORAGE_ENABLED=1
SIGNAL_CANDIDATE_DATA_MAX_ITEMS=1000
SIGNAL_CANDIDATE_DATA_HISTORY_LIMIT=30
SIGNAL_FINAL_DECISION_STABILITY_ENABLED=1
SIGNAL_FINAL_DECISION_STABILITY_SECONDS=120
SIGNAL_MARKET_DATA_LATEST_ENABLED=1
SIGNAL_MARKET_DATA_LATEST_MAX_ITEMS=1000
SIGNAL_MARKET_DATA_LATEST_ITEMS_MAX_LIMIT=1000
SIGNAL_LIVE_STATE_STORAGE_ENABLED=1
SIGNAL_LIVE_STATE_RETAIN_SECONDS=900
SIGNAL_PERFORMANCE_AUTO_UPDATE=1
SIGNAL_PERFORMANCE_MIN_AGE_MINUTES=60
```

화면에서 `뉴스 소스 상태 > 글로벌 출처`가 `제한`으로 표시되면 GDELT 요청 제한입니다. 몇 분 뒤 새로고침하거나 후보 조회 수를 1로 유지한 상태에서 다시 확인합니다.

메인 화면의 `후보 풀` 카드는 DB에 누적된 상시 감시 후보 중 현재 살아 있는 후보 수, 진입/검증/눌림 상태, 상위 후보, 사후 성과를 보여줍니다. 스냅샷 저장 후에는 60분 이상 지난 기존 스냅샷의 기준가와 현재가를 자동 비교하고, 그 결과가 후보 풀에 누적됩니다. `성과` 화면을 열어 수동으로도 같은 검증을 다시 실행할 수 있습니다. 승률과 평균 변화는 다음 후보 압축·정렬에 제한적으로 반영되며, 리스크와 가격 반응 기준을 통과하지 못한 종목을 매수 후보로 강제로 올리지는 않습니다.
...
선택값은 차트 20개, 호가/체결 10개 우선 보강을 기준으로 둡니다. 호가/체결은 요청 비용이 커서 상한값을 명시적으로 올리기 전에는 10개를 넘지 않습니다.

```text
TOSS_CANDLE_MAX_CANDIDATES=20
TOSS_ORDERBOOK_MAX_CANDIDATES=10
TOSS_ORDERBOOK_CANDIDATES_MAX_LIMIT=10
TOSS_TRADES_MAX_CANDIDATES=10
TOSS_TRADES_CANDIDATES_MAX_LIMIT=10
...
SIGNAL_SCHEDULER_INTERVAL_SECONDS=60
SIGNAL_SCHEDULER_INTERVAL_MIN_SECONDS=60
```
