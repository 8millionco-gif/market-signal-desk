...
자동 후보 생성 기본값은 아래처럼 둡니다. 네이버 뉴스 활성화 전에는 유니버스 기본 점수로 후보를 구성하고, 네이버 뉴스 활성화 후에는 뉴스 스캔 결과가 후보 선정에 반영됩니다.

```text
SIGNAL_AUTO_CANDIDATES_ENABLED=1
SIGNAL_AUTO_CANDIDATE_LIMIT=6
SIGNAL_DISCOVERY_MAX_SYMBOLS=12
