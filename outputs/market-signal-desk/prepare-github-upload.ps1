...
function Resolve-WorkspaceRoot {
  ...
  foreach ($candidate in $candidates) {
    ...
    if ((Test-Path (Join-Path $resolved "render.yaml")) -and (Test-Path (Join-Path $resolved "outputs\market-signal-desk\server.py"))) {
      return $resolved
    }
  }

  throw "Workspace root not found. Run from outputs\market-signal-desk or the repository root."
}
...
try {
  ...
  $candidateFiles = $candidateFiles | Where-Object {
    $_ -ne "prepare-github-upload.ps1" -and $_ -ne "outputs/market-signal-desk/prepare-github-upload.ps1"
  }
